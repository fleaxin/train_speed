...
#define rgn_check_handle_num_return(handle_num) \
    do { \
        if (((handle_num) < SAMPLE_RGN_HANDLE_NUM_MIN) || ((handle_num) > SAMPLE_RGN_HANDLE_NUM_MAX)) { \
            err_print("handle_num(%d) should be in [%d, %d].\n", \
...
typedef enum {
    ...
    SONY_IMX347_SLAVE_MIPI_1080P_30FPS_12BIT,   //added by lewis
...
td_s32 venc_stop_get_stream_push_and_save_file(td_s32 chn_num);
