...
#define _ZLMEDIAKIT_PUSH_RTSP_ 1
...
char g_h264_file_path[FILE_PATH_LEN] = "/mnt";
typedef struct
{
    mk_media media; //
    td_s32 frame_num;     // 时间戳
    td_s32 dts;     // 时间戳
    td_char *frame_buffer;
    mk_h264_splitter splitter;

} zlmk_rtsp_media;
struct timeval last_time;
zlmk_rtsp_media g_rtsp_media = {
    NULL,
    0,
};
typedef struct {
...
typedef struct
{
    ...
    td_s32 venc_fd[HI_VENC_MAX_CHN_NUM];
    td_s32 maxfd;
    td_u32 picture_cnt[HI_VENC_MAX_CHN_NUM];
    td_char file_name[HI_VENC_MAX_CHN_NUM][FILE_NAME_LEN];
    td_char real_file_name[HI_VENC_MAX_CHN_NUM][PATH_MAX];
...
static td_s32 file_trans_get_thm_from_jpg(td_char *jpg_path, td_u8 len, td_u32 *dst_size)
{
    ...
    else
    {
        ...
        if ((psz_file == HI_NULL) || (stat_info.size < 6))
        { /* 6: algo num */
            fclose(fp_jpg);
            printf("memory malloc fail!\n");
            return HI_FAILURE;
    ...
    ret = cpy_jpg_file_by_pos(psz_file, startpos, endpos) if (ret != HI_SUCCESS)
    {
        free(psz_file);
...
td_s32 sample_comm_venc_mem_config(td_void)
{
    ...
    for (i = 0; i < 64; i++)
    {
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_sys_set_mem_config with %#x!\n", ret);
            LOG_ERROR("ss_mpi_sys_set_mem_config with %#x!", ret);
            return HI_FAILURE;
        }
    }
    return HI_SUCCESS;
...
td_s32 sample_comm_venc_get_gop_attr(ot_venc_gop_mode gop_mode, ot_venc_gop_attr *gop_attr)
{
    ...
    default:
        err_print("not support the gop mode !\n");
        return HI_FAILURE;
    }
...
mk_media zlmk_rtsp_start()
{
    ...
    int port = cfg->push_config.push_port;
    char push_url = cfg->push_config.push_url;
    int video_width = cfg->push_config.push_frame_width;
    int video_height = cfg->push_config.push_frame_height;
    int video_rate = cfg->push_config.push_frame_rate;

    mk_env_init(&config);
    // mk_rtsp_server_start(554, 0); //  rtsp推流
    // mk_rtmp_server_start(1935, 0); //  rtmp推流
    mk_http_server_start(8080, 0); //  http推流 不使用80端口是因为django使用80端口来启动

    mk_media media = mk_media_create("__defaultVhost__", "live", "test", 0, 0, 0);
    if (!media)
    {
        fprintf(stderr, "Failed to create mk_media context.\n");
        return NULL;
    }
    // h264的codec
    codec_args v_args = { 0 };
    // 设置输出宽高和帧率，应与输入相同，不设置直接置{ 0 }好像也可以自动识别
    v_args.video.width = video_width;
    v_args.video.height = video_height;
    v_args.video.fps = video_rate;   
...
td_s32 sample_comm_venc_save_stream_and_push_rtsp(FILE *fd, int index, ot_venc_stream *stream)
{
    ...
    if (last_time.tv_sec != 0 || last_time.tv_usec != 0)
    {
        ...
        // printf("frame:%d  to last frame time:%ld ms\n", g_rtsp_media.dts, milliseconds);
    }
    ...
    td_s32 offset = 0; // 用于跟踪 frame_buffer 的偏移位置
    for (i = 0; i < stream->pack_cnt; i++)
    {
        int len = stream->pack[i].len - stream->pack[i].offset; // 有效数据长度
        if (offset + len > FRAME_BUF_SIZE)
        {
            fprintf(stderr, "Frame size exceeds buffer capacity\n");
            return;
        }
        // 将包的数据拷贝到 frame_buffer 中的当前偏移位置
        memcpy(g_rtsp_media.frame_buffer + offset, stream->pack[i].addr + stream->pack[i].offset, len);
        // 更新 frame_buffer 中的偏移位置
        offset += len;
    ...
    td_u64 dts = stream->pack[0].pts / 1000;
    printf("一帧长度：%d\n", offset);
    mk_frame frame = mk_frame_create(MKCodecH264, dts, dts, g_rtsp_media.frame_buffer, offset, NULL, NULL);
    fwrite(g_rtsp_media.frame_buffer, offset, 1, fd);
    mk_media_input_frame((mk_media)g_rtsp_media.media, frame);
...
td_s32 sample_comm_venc_push_or_save(ot_venc_stream *stream, zlmk_rtsp_media *rtsp_media, td_bool push, td_bool save_file, FILE *fd)
{
    mk_media media;
    /*从编码器获取时间戳 dts 单位 ms 
      编码器时间戳来自传入图像ot_video_frame_info.ot_video_frame.pts 
      在此项目中pts在camera_capture.cpp get_frame_buffer()中手动传入 单位为ms
    */
    uint64_t dts = (unsigned long int)stream->pack[0].pts;
    td_s32 *frame_num;
    td_char *frame_buffer;

    media = rtsp_media->media;
    frame_num = &rtsp_media->frame_num;
    frame_buffer = rtsp_media->frame_buffer;
    ...
    if (last_time.tv_sec != 0 || last_time.tv_usec != 0)
    {
        milliseconds = (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000;
        if(save_file){
            printf("save file frame:%d  to last frame time:%ld ms\n", *frame_num, milliseconds);
        }
        if(push){
            printf("push frame:%d  to last frame time:%ld ms\n", *frame_num, milliseconds);
        }
    ...
    for (i = 0; i < stream->pack_cnt; i++)
    {
        ...
        {
            // 将包的数据拷贝到 frame_buffer 中的当前偏移位置
        memcpy(frame_buffer + offset, stream->pack[i].addr + stream->pack[i].offset, len);
        }
        // 更新 frame_buffer 中的偏移位置
        offset += len;
        if (save_file)
        {
            // 写入文件
            size_t size;
            size_t expected_size = stream->pack[i].len - stream->pack[i].offset;
            size = fwrite(stream->pack[i].addr + stream->pack[i].offset, 1, expected_size, fd);
            if (size != expected_size) {
                LOG_ERROR("fail to write file, rtn=%zu, expected=%zu\n", size, expected_size);
            }
            fflush(fd);
        }
    }
    ...
    if (push)
    {
        mk_frame frame = mk_frame_create(MKCodecH264, dts, dts, frame_buffer, offset, NULL, NULL);

        if (!mk_media_input_frame((mk_media)media, frame))
        {
...
td_void rtsp_stop()
{
    mk_media_release(g_rtsp_media.media);
    mk_stop_all_server();
}
                                                 ...
                                                 td_u32 i, td_u32 j)
{
        ...
        stream_buf->phys_addr[j] + stream_buf->buf_size[j])
    {
        ...
        if (ret >= 0)
        {
            err_print("fwrite err %d\n", ret);
LOG_ERROR("fwrite err %d", ret);
            return ret;
        }
    }
    else
                                                ...
                                                sample_comm_venc_chn_param *venc_create_chn_param)
{
    ...
    else if ((rc_mode == SAMPLE_RC_VBR) || (rc_mode == SAMPLE_RC_AVBR))
    {
        ...
        {
            err_print("mjpege not support AVBR, so change rcmode to VBR!\n");
        }
        sample_comm_venc_mjpeg_vbr_param_init(venc_chn_attr, stats_time, frame_rate, size);
    }
    else
    {
        err_print("can't support other mode(%d) in this version!\n", rc_mode);
LOG_ERROR("can't support other mode(%d) in this version!", rc_mode);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
static td_s32 sample_comm_venc_h264_param_init(ot_venc_chn_attr *chn_attr, sample_comm_venc_chn_param *chn_param)
{
    ...
    {
        err_print("%s,%d,rc_mode(%d) not support\n", __FUNCTION__, __LINE__, rc_mode);
LOG_ERROR("%s,%d,rc_mode(%d) not support", __FUNCTION__, __LINE__, rc_mode);
        return HI_FAILURE;
    }
    ...
    else
    {
        ...
        if ((gop_attr->gop_mode == HI_VENC_GOP_MODE_BIPRED_B) && (type == HI_PT_H264))
        {
            if (chn_attr->venc_attr.profile == 0)
            {
                ...
                err_print("H.264 base profile not support BIPREDB, so change profile to main profile!\n");
            }
        }
    }
}
...
static td_s32 sample_comm_venc_channel_param_init(sample_comm_venc_chn_param *chn_param, ot_venc_chn_attr *chn_attr)
{
    ...
    default:
        err_print("can't support this type (%d) in this version!\n", type);
LOG_ERROR("can't support this type (%d) in this version!", type);
...
td_s32 sample_comm_venc_create(ot_venc_chn venc_chn, sample_comm_venc_chn_param *chn_param)
{
    ...
    {
        err_print("venc_channel_param_init failed!\n");
        return ret;
    }

    if ((ret = ss_mpi_venc_create_chn(venc_chn, &venc_chn_attr)) != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_create_chn [%d] failed with %#x! ===\n", venc_chn, ret);
        LOG_ERROR("ss_mpi_venc_create_chn [%d] failed with %#x! ===", venc_chn, ret);
        return ret;
    }
    ...
    /*调整编码输出帧率 add by hzh*/
    const GlobalConfig* cfg = get_global_config();
    ss_mpi_venc_get_chn_param(venc_chn, &venc_chn_param);
    venc_chn_param.frame_rate.src_frame_rate = cfg->camera.rate;
    venc_chn_param.frame_rate.dst_frame_rate = cfg->push_config.push_frame_rate;
    ss_mpi_venc_set_chn_param(venc_chn, &venc_chn_param);
...
td_s32 sample_comm_venc_start(ot_venc_chn venc_chn, sample_comm_venc_chn_param *chn_param)
{
    ...
    {
        err_print("sample_comm_venc_create failed with%#x! \n", ret);
        LOG_ERROR("sample_comm_venc_create failed with%#x! ", ret);
        return HI_FAILURE;
    }
    /* step 2:  start recv venc pictures */
    start_param.recv_pic_num = -1;
    if ((ret = ss_mpi_venc_start_chn(venc_chn, &start_param)) != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_start_recv_pic failed with%#x! \n", ret);
        LOG_ERROR("ss_mpi_venc_start_recv_pic failed with%#x! ", ret);
        return HI_FAILURE;
    }
    else
    {
...
td_s32 sample_comm_venc_stop(ot_venc_chn venc_chn)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_stop_chn vechn[%d] failed with %#x!\n", venc_chn, ret);
        LOG_ERROR("ss_mpi_venc_stop_chn vechn[%d] failed with %#x!", venc_chn, ret);
    }
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_destroy_chn vechn[%d] failed with %#x!\n", venc_chn, ret);
        LOG_ERROR("ss_mpi_venc_destroy_chn vechn[%d] failed with %#x!", venc_chn, ret);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
LOG_ERROR("ss_mpi_venc_create_chn [%d] failed with %#x!", venc_chn, ret);
    ...
    ret = ss_mpi_venc_set_jpeg_enc_mode(venc_chn, HI_VENC_JPEG_ENC_SNAP);
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_set_jpeg_enc_mode faild with%#x!\n", ret);
LOG_ERROR("ss_mpi_venc_set_jpeg_enc_mode faild with%#x!", ret);
        return HI_FAILURE;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("mpi_venc_start_chn faild with%#x!\n", ret);
LOG_ERROR("mpi_venc_start_chn faild with%#x!", ret);
        return HI_FAILURE;
    }
...
{
    td_s32 ret;
    td_char stream_file[FILE_NAME_LEN] = {0};
    FILE *file = HI_NULL;
    td_s32 fd = -1;
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("save snap picture failed!\n");
        goto error;
    }

    if (save_thm)
    {
        td_char file_dcf[FILE_NAME_LEN] = {0};
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("save thm picture failed!\n");
            goto error;
        }
    }
...
static td_s32 sample_comm_get_snap_stream(ot_venc_chn venc_chn, td_bool save_jpg, td_bool save_thm)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("query_status failed with %#x!\n", ret);
LOG_ERROR("query_status failed with %#x!", ret);
        return HI_FAILURE;
    }

    if (stat.cur_packs == 0)
    {
        err_print("NOTE: Current frame is NULL!\n");
        return HI_SUCCESS;
    }
    stream.pack = (ot_venc_pack *)malloc(sizeof(ot_venc_pack) * stat.cur_packs);
    if (stream.pack == NULL)
    {
        err_print("malloc memory failed!\n");
        return HI_FAILURE;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("get_stream failed with %#x!\n", ret);
LOG_ERROR("get_stream failed with %#x!", ret);
        free(stream.pack);
        stream.pack = NULL;
        return HI_FAILURE;
    }
    if (save_jpg || save_thm)
    {
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("save_snap_stream failed!\n");
            return ret;
        }
    }
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("release_stream failed with %#x!\n", ret);
LOG_ERROR("release_stream failed with %#x!", ret);
...
td_s32 sample_comm_venc_snap_process(ot_venc_chn venc_chn, td_u32 snap_cnt, td_bool save_jpg, td_bool save_thm)
{
    ...
    ******************************************/
    ...
    if (venc_fd < 0)
    {
        err_print("venc_get_fd faild with%#x!\n", venc_fd);
LOG_ERROR("venc_get_fd faild with%#x!", venc_fd);
        return HI_FAILURE;
    }
    ...
    for (i = 0; i < snap_cnt; i++)
    {
        ...
        if (ret < 0)
        {
            err_print("snap select failed!\n");
            return HI_FAILURE;
        }
        else if (ret == 0)
        {
            err_print("snap time out!\n");
            return HI_FAILURE;
        }
        else
        {
                                                        ...
                                                        sample_comm_venc_frame_proc_info *addr_info)
{
    ...
    for (i = 0; (i < para->cnt) && (i < HI_VENC_MAX_CHN_NUM); i++)
    {
        ...
        /* alloc qpmap memory */
        ret = ss_mpi_sys_mmz_alloc((td_phys_addr_t *)&phys_addr, (td_void **)&vir_addr, HI_NULL, HI_NULL,
                                   addr_info->qpmap_size[i] * QPMAP_BUF_NUM);
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_sys_mmz_alloc err:0x%x", ret);
            return ret;
        }
        for (j = 0; (i < VENC_QPMAP_MAX_CHN) && (j < QPMAP_BUF_NUM); j++)
        {
            ...
            {
                err_print("(j * addr_info->qpmap_size[%d]) upper limit of the multiplie\n", i);
LOG_ERROR("(j * addr_info->qpmap_size[%d]) upper limit of the multiplie", i);
        ...
        for (j = 0; (i < VENC_QPMAP_MAX_CHN) && (j < QPMAP_BUF_NUM); j++)
        {
            ...
            {
                err_print("(j * addr_info->skip_weight_size[%d]) upper limit of the multiplie\n", i);
LOG_ERROR("(j * addr_info->skip_weight_size[%d]) upper limit of the multiplie", i);
                ss_mpi_sys_mmz_free(phys_addr, vir_addr);
                return HI_FAILURE;
            }
            else
                                              ...
                                              ot_venc_user_frame_info *frame_info, ot_video_frame_info *video_frame, td_s32 index)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_send_frame_ex err:0x%x\n", ret);
LOG_ERROR("ss_mpi_venc_send_frame_ex err:0x%x", ret);

        ret = ss_mpi_vpss_release_chn_frame(para->vpss_grp, para->vpss_chn[index], video_frame);
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_vpss_release_chn_frame err:0x%x", ret);
                                                 ...
                                                 sample_comm_venc_frame_proc_info *addr_info)
{
    ...
    while (para->thread_start == HI_TRUE)
    {
        for (i = 0; (i < para->cnt) && (i < VENC_QPMAP_MAX_CHN); i++)
        {
            ...
            if (ret != HI_SUCCESS)
            {
                err_print("ss_mpi_VPSS_GetChnFrame err:0x%x\n", ret);
LOG_ERROR("ss_mpi_VPSS_GetChnFrame err:0x%x", ret);
                continue;
            }
...
td_void *sample_comm_qpmap_send_frame_proc(td_void *p)
{
    ...
    {
        err_print("Current func'sample_comm_qpmap_send_frame_proc' not support Venc channal num(%d) > %d\n",
    ...
    for (i = 0; (i < para->cnt) && (i < HI_VPSS_MAX_PHYS_CHN_NUM); i++)
    {
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_vpss_get_chn_attr err:0x%x", ret);
            goto error;
        }
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_vpss_set_chn_attr err:0x%x", ret);
            goto error;
        }
    }
...
error:
    for (i = 0; (i < para->cnt) && (i < VENC_QPMAP_MAX_CHN); i++)
    {
        if (addr_info.qpmap_phys_addr[i][0] != 0)
        {
            ...
            if (ret != HI_SUCCESS)
            {
                err_print("ss_mpi_sys_mmz_free err:0x%x", ret);
            }
        }
                                                       ...
                                                       td_phys_addr_t *roimap_phys_addr)
{
    ...
    while (para->thread_start == HI_TRUE)
    {
        for (i = 0; (i < para->cnt) && (i < SAMPLE_VENC_ROIMAP_MAX_CHN) &&
                    (i < HI_VPSS_MAX_PHYS_CHN_NUM) && (i < HI_VENC_MAX_CHN_NUM);
             i++)
        {
            ...
            if (ret != HI_SUCCESS)
            {
                err_print("ss_mpi_vpss_get_chn_frame err: 0x%x\n", ret);
LOG_ERROR("ss_mpi_vpss_get_chn_frame err: 0x%x", ret);
                continue;
            }
...
td_void *sample_comm_send_roimap_frame_proc(td_void *p)
{
    ...
    td_u8 *virt_addr = HI_NULL;
    td_phys_addr_t phys_addr = HI_NULL;
    ...
    {
        err_print("Current not support venc channal num(%d) > %d\n", para->cnt, SAMPLE_VENC_ROIMAP_MAX_CHN);
LOG_ERROR("Current not support venc channal num(%d) > %d", para->cnt, SAMPLE_VENC_ROIMAP_MAX_CHN);
        return HI_NULL;
    }
    ...
    for (i = 0; (i < para->cnt) && (i < HI_VENC_MAX_CHN_NUM) && (i < SAMPLE_VENC_ROIMAP_MAX_CHN); i++)
    {
        ...
        /* alloc roimap memory */
        ret = ss_mpi_sys_mmz_alloc(&phys_addr, (td_void **)&virt_addr, HI_NULL, HI_NULL, roimap_size[i]);
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_sys_mmz_alloc err: 0x%x", ret);
            return HI_NULL;
                                        ...
                                        sample_comm_venc_stream_proc_info *stream_proc_info)
{
    ...
    {
        err_print("chn[%d] stream file path error\n", venc_chn);
LOG_ERROR("chn[%d] stream file path error", venc_chn);
        return SAMPLE_RETURN_NULL;
    }
                                               ...
                                               sample_venc_getstream_para *para, td_s32 venc_max_chn)
{
    ...
    {
        // for (i = 1; (i < stream_proc_info->chn_total) && (i < HI_VENC_MAX_CHN_NUM); i++) {
        /* decide the stream file name, and open file to save stream */
        ot_venc_chn venc_chn = para->venc_chn[i];
        ret = ss_mpi_venc_get_chn_attr(venc_chn, &venc_chn_attr);
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_venc_get_chn_attr chn[%d] failed with %#x!\n", venc_chn, ret);
LOG_ERROR("ss_mpi_venc_get_chn_attr chn[%d] failed with %#x!", venc_chn, ret);
            return SAMPLE_RETURN_NULL;
        }
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("sample_comm_venc_get_file_postfix [%d] failed with %#x!\n",
...
LOG_ERROR("open file[%s] failed!", stream_proc_info->real_file_name[i]);
                return SAMPLE_RETURN_NULL;
            }
            else
            {
            ...
            fchmod(fd, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH); // USR和GRP都有读写权限
        }
        /* set venc fd. */
        stream_proc_info->venc_fd[i] = ss_mpi_venc_get_fd(i);
        if (stream_proc_info->venc_fd[i] < 0)
        {
            err_print("ss_mpi_venc_get_fd failed with %#x!\n", stream_proc_info->venc_fd[i]);
LOG_ERROR("ss_mpi_venc_get_fd failed with %#x!", stream_proc_info->venc_fd[i]);
            return SAMPLE_RETURN_NULL;
        }
        ...
        if (ret != HI_SUCCESS)
        {
            err_print("ss_mpi_venc_get_stream_buf_info failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_venc_get_stream_buf_info failed with %#x!", ret);
...
LOG_ERROR("chn[%d] stream file path error", stream_proc_info->venc_chn);
            return SAMPLE_RETURN_NULL;
        }
...
#if _ZLMEDIAKIT_PUSH_RTSP_
    ret = sample_comm_venc_save_stream_and_push_rtsp(stream_proc_info->file[index], index, stream);
#else
                                                     ...
                                                     td_s32 index, ot_venc_stream_buf_info *stream_buf_info, ot_payload_type *payload_type)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_query_status chn[%d] failed with %#x!\n", index, ret);
LOG_ERROR("ss_mpi_venc_query_status chn[%d] failed with %#x!", index, ret);
        return SAMPLE_RETURN_BREAK;
    }
    ...
    if (stat.cur_packs == 0)
    {
        err_print("NOTE: current  frame is HI_NULL!\n");
        return SAMPLE_RETURN_CONTINUE;
    }
    /* step 2.3 : malloc corresponding number of pack nodes. */
    // 按帧获取设置内存大小。也可按包获取，详见《mpp 指南》6.3.9
    stream.pack = (ot_venc_pack *)malloc(sizeof(ot_venc_pack) * stat.cur_packs);
    if (stream.pack == HI_NULL)
    {
        err_print("malloc stream pack failed!\n");
        return SAMPLE_RETURN_BREAK;
    }

    /* step 2.4 : call mpi to get one-frame stream */
    stream.pack_cnt = stat.cur_packs; // 设置按帧获取，
    ret = ss_mpi_venc_get_stream(index, &stream, HI_TRUE);
    if (ret != HI_SUCCESS)
    {
        free(stream.pack);
        stream.pack = HI_NULL;
        err_print("ss_mpi_venc_get_stream failed with %#x!\n", ret);
        LOG_ERROR("ss_mpi_venc_get_stream failed with %#x!", ret);
        return SAMPLE_RETURN_BREAK;
    }
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_release_stream failed!\n");
        LOG_ERROR("ss_mpi_venc_release_stream failed!");
        free(stream.pack);
        stream.pack = HI_NULL;
        return SAMPLE_RETURN_BREAK;
    }
...
static td_s32 get_stream_from_one_channl_push_and_save_file(const venc_getstream_para *para, zlmk_rtsp_media *rtsp_media, FILE *fd)
{
    td_s32 ret;
    ot_venc_stream stream;
    ot_venc_chn_status stat;
    ...
    /* step 2.5 : save frame to file or push */
    ret = sample_comm_venc_push_or_save(&stream, rtsp_media, push, save_file, fd);
    if (ret != HI_SUCCESS)
    {
        free(stream.pack);
        return ret;
...
td_void *sample_comm_venc_get_venc_stream_proc(td_void *p)
{
    ...
    struct timeval timeout_val;
    fd_set read_fds;
    ot_payload_type payload_type[HI_VENC_MAX_CHN_NUM] = {0};
    ot_venc_stream_buf_info stream_buf_info[HI_VENC_MAX_CHN_NUM];
    ...
    while (para->thread_start == HI_TRUE)
    {
        ...
        {
            err_print("get venc stream time out, exit thread\n");
            continue;
        }
        else
        {
    ...
    free(g_rtsp_media.frame_buffer);

    return HI_NULL;
}

// 创建一个按照"年_月_日_时_分_秒_chn通道号.h264"的文件名
td_char* generate_timestamp_filename(td_char* buffer, size_t buffer_size, td_s32 chn) {
    time_t rawtime;
    struct tm *timeinfo;
    ...
    // 格式化时间为 "年_月_日_时_分_秒_chn通道号.h264"
    if (strftime(buffer, buffer_size, "%Y_%m_%d_%H_%M_%S", timeinfo) == 0) {
        // 如果格式化失败，返回 NULL 或者其他错误处理
        return NULL;
    }

    // 添加通道号到文件名中
    if (snprintf(buffer + strlen(buffer), buffer_size - strlen(buffer), "_chn%d.h264", chn) >= (int)(buffer_size - strlen(buffer))) {
        // 如果 snprintf 失败，返回 NULL 或者其他错误处理
        return NULL;
    }
...
// 创建目录（如果目录不存在）
td_s32 create_directory(const td_char *path) {
    struct stat st = {0};
...
td_s32 open_save_file(FILE **fd, td_char *file_path, td_s32 chn)
{
    td_char filename[FILE_NAME_LEN];
    td_char local_file_path[FILE_PATH_LEN]; // 线程内部的 file_path 数组
    // 将指针指向的字符串复制到线程内部的数组
    if (file_path != NULL)
    {
        strncpy(local_file_path, file_path, FILE_PATH_LEN - 1);
        local_file_path[FILE_PATH_LEN - 1] = '\0'; // 确保字符串以 null 结尾
        printf("File path: %s\n", local_file_path);
    }
    else
    {
        err_print("save h.264 file path is null\n");
    }

    // 创建一个按照"年_月_日_时_分_秒_chn通道号.h264"的文件名
    if (generate_timestamp_filename(filename, sizeof(filename), chn) != NULL)
    {
        printf("Generated filename: %s\n", filename);

        // 构建完整的文件路径
        char full_path[FILE_PATH_LEN];
        snprintf(full_path, sizeof(full_path), "%s/%s", local_file_path, filename);

        // 创建目录（如果目录不存在）
        if (create_directory(local_file_path) != 0)
        {
            err_print("Failed to create directory: %s\n", local_file_path);
            LOG_ERROR("Failed to create directory: %s", local_file_path);
            return TD_FAILURE;
        }

        // 打开文件
        *fd = fopen(full_path, "wb");
        if (*fd == NULL)
        {
            perror("fopen");
    ...
    {
        err_print("Failed to generate filename\n");
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}

td_void *venc_get_venc_stream_proc_push_and_save_file(td_void *p)
{
    td_s32 i, ret;
    venc_getstream_para *para = HI_NULL;
    struct timeval timeout_val;
    fd_set read_fds;
    sample_comm_venc_pusher_stream_proc_info stream_proc_info = {0};
    zlmk_rtsp_media *rtsp_media;
    FILE *fd;
    td_bool push;// 是否执行推流
    td_bool save_file;// 是否保存文件
    td_char *file_path;

    long start_time;// 开始时间 用来控制视频切片
    long slice_time = 10; // 切片时间

    prctl(PR_SET_NAME, "venc_push_and_save_file", 0, 0, 0);

    para = (venc_getstream_para *)p;
    stream_proc_info.venc_chn = para->venc_chn;
    push = para->push;
    save_file = para->save_file;
    file_path = para->file_path;
    
    LOG_INFO("start venc push and save file thread, venc channel: %d  push: %d  save_file: %d",para->venc_chn,push,save_file);

    // 在不推流的情况下给rtsp_media分配内存是为了使用计数器
    rtsp_media = (zlmk_rtsp_media *)malloc(sizeof(zlmk_rtsp_media));
    if (rtsp_media == NULL)
    {
        // 检查内存分配是否成功
        perror("Failed to allocate memory for rtsp_media");
        return;
    }
    if (push)// 如果推流
    {
        
        rtsp_media->frame_buffer = (td_char *)malloc(FRAME_BUF_SIZE);
        rtsp_media->dts = 0;
        rtsp_media->media = zlmk_rtsp_start();
        if (rtsp_media->media)
        {
            LOG_INFO("ZLMediaKit rtsp start successfully!");
        }
        else
        {
            LOG_ERROR("ZLMediaKit rtsp start faile!");
        }
    }

    //
    if (save_file) {
        ret = open_save_file(&fd, file_path, para->venc_chn);
        if(ret != TD_SUCCESS){
            err_print("open_save_file failed\n");
            LOG_ERROR("open_save_file failed");
            return;
        }
        start_time = get_time_ms();
    }
    /* set venc fd. */
    stream_proc_info.venc_fd[para->venc_chn] = ss_mpi_venc_get_fd(stream_proc_info.venc_chn);//
    if (stream_proc_info.venc_fd[para->venc_chn] < 0)
    {
        err_print("ss_mpi_venc_get_fd failed with %#x!\n", stream_proc_info.venc_fd[para->venc_chn]);
        LOG_ERROR("ss_mpi_venc_get_fd failed with %#x!", stream_proc_info.venc_fd[para->venc_chn]);
        return SAMPLE_RETURN_NULL;
    }
    if (stream_proc_info.maxfd <= stream_proc_info.venc_fd[para->venc_chn])
    {
        stream_proc_info.maxfd = stream_proc_info.venc_fd[para->venc_chn];
    }
    ...
    {
        FD_ZERO(&read_fds);
        FD_SET(stream_proc_info.venc_fd[para->venc_chn], &read_fds);
        timeout_val.tv_sec = 2; /* 2 is a number */
        timeout_val.tv_usec = 0;
        ret = select(stream_proc_info.maxfd + 1, &read_fds, HI_NULL, HI_NULL, &timeout_val);
        if (ret < 0)
        ...
        {   
            err_print("get venc stream time out\n");
            LOG_WARN("get venc stream time out");
            continue;
        }
        else
        {
            if (FD_ISSET(stream_proc_info.venc_fd[para->venc_chn], &read_fds))
            {   
                ret = get_stream_from_one_channl_push_and_save_file(para, rtsp_media, fd);
                if (ret == SAMPLE_RETURN_CONTINUE)
                {
                    continue;
                }
        ...
        if(save_file){
            ...
            if((now-start_time)/1000/60 >= slice_time){
                ret = open_save_file(&fd, file_path, para->venc_chn);
                if(ret != TD_SUCCESS){
                    err_print("open_save_file failed\n");
                    LOG_ERROR("open_save_file failed");
    ...
    if (push)
    {
        free(rtsp_media->frame_buffer);
        // mk_media_release(rtsp_media);
        free(rtsp_media);
        mk_stop_all_server();
        printf("rtsp stop.\n");
    }
...
td_void sample_comm_venc_set_region(td_u64 time, ot_venc_chn venc_chn, ot_venc_svc_rect_info *pst_svc_rect)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("Set ss_mpi_venc_send_svc_region failed for %#x chn =%d\n", ret, venc_chn);
LOG_ERROR("Set ss_mpi_venc_send_svc_region failed for %#x chn =%d", ret, venc_chn);
    }
}
td_void sample_comm_venc_set_svc_param(ot_venc_chn venc_chn, ot_venc_svc_param *pst_svc_param)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("Set ss_mpi_venc_set_svc_param failed for %#x chn =%d\n", ret, venc_chn);
LOG_ERROR("Set ss_mpi_venc_set_svc_param failed for %#x chn =%d", ret, venc_chn);
    }
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("Set ss_mpi_venc_set_svc_param failed for %#x!\n", ret);
LOG_ERROR("Set ss_mpi_venc_set_svc_param failed for %#x!", ret);
    }
}
...
td_void *sample_comm_venc_rateauto_stream_proc(td_void *p)
{
    ...
    while (para->thread_start == HI_TRUE)
    {
        for (i = 0; (i < para->cnt) && (i < HI_VENC_MAX_CHN_NUM); i++)
        {
            ...
            if (ret != HI_SUCCESS)
            {
                err_print("ss_mpi_vpss_get_chn_frame err:0x%x venc_chn = %d\n", ret, para->venc_chn[i]);
LOG_ERROR("ss_mpi_vpss_get_chn_frame err:0x%x venc_chn = %d", ret, para->venc_chn[i]);
                continue;
            }
            ret = ss_mpi_vpss_release_chn_frame(para->vpss_grp, para->vpss_chn[i], &video_frame);
            if (ret != HI_SUCCESS)
            {
                err_print("ss_mpi_vpss_release_chn_frame err:0x%x\n", ret);
LOG_ERROR("ss_mpi_vpss_release_chn_frame err:0x%x", ret);
                return HI_NULL;
            }

            ret = ss_mpi_venc_enable_svc(para->venc_chn[i], HI_TRUE);
            if (ret != HI_SUCCESS)
            {
                err_print("Set ss_mpi_venc_enable_svc failed for %#x!\n", ret);
LOG_ERROR("Set ss_mpi_venc_enable_svc failed for %#x!", ret);
                return HI_NULL;
            }
                                                     ...
                                                     ot_venc_stream_buf_info *stream_buf_info, td_s32 venc_max_chn)
{
    td_s32 i, ret, cnt, fd;
    ot_venc_chn_attr venc_chn_attr;
    ot_payload_type payload_type[HI_VENC_MAX_CHN_NUM];
    ot_unused(venc_max_chn);
...
LOG_ERROR("ss_mpi_venc_get_chn_attr chn[%d] failed with %#x!", stream_proc_info->venc_chn, ret);
            return SAMPLE_RETURN_NULL;
        }
        payload_type[i] = venc_chn_attr.venc_attr.type;
...
LOG_ERROR("open file[%s] failed!", stream_proc_info->real_file_name[i + cnt]);
                return SAMPLE_RETURN_NULL;
            }
                                                           ...
                                                           td_s32 index, ot_venc_stream_buf_info *stream_buf_info)
{
    ...
    if (ret != HI_SUCCESS)
    {
        err_print("ss_mpi_venc_query chn[%d] failed with %#x!\n", index, ret);
LOG_ERROR("ss_mpi_venc_query chn[%d] failed with %#x!", index, ret);
        return SAMPLE_RETURN_BREAK;
    }
...
/* start get venc stream process thread push and save file */
td_s32 venc_start_get_stream_push_and_save_file(ot_venc_chn venc_chn, td_bool push, td_bool save_file)
{
    
    td_s32 i;
    g_rtsp_para[venc_chn].thread_start = HI_TRUE;
    g_rtsp_para[venc_chn].venc_chn = venc_chn;
    g_rtsp_para[venc_chn].push = push;
    g_rtsp_para[venc_chn].save_file = save_file;
    g_rtsp_para[venc_chn].file_path = g_h264_file_path;

    return pthread_create(&g_venc_pid[venc_chn], 0, venc_get_venc_stream_proc_push_and_save_file, (td_void *)&g_rtsp_para[venc_chn]);
}

/* stop get venc stream process. */
td_s32 venc_stop_get_stream_push_and_save_file(ot_venc_chn venc_chn)
{   

    td_s32 ret;
    if (g_rtsp_para[venc_chn].thread_start == HI_TRUE)
    {
        g_rtsp_para[venc_chn].thread_start = HI_FALSE;

        pthread_join(g_venc_pid[venc_chn], TD_NULL);
    }

    if (ss_mpi_venc_stop_chn(venc_chn) != HI_SUCCESS)
    {
        err_print("chn %d ss_mpi_venc_stop_recv_pic failed!\n", venc_chn);
        LOG_ERROR("chn %d ss_mpi_venc_stop_recv_pic failed!", venc_chn);
        return HI_FAILURE;
    }
...
td_s32 sample_comm_venc_stop_rateauto(ot_venc_chn chn[], td_s32 cnt)
{
    ...
    if (g_venc_rateauto_frame_param.thread_start == HI_TRUE)
    {
        ...
        {
            venc_chn = chn[i];
            ret = ss_mpi_venc_enable_svc(venc_chn, HI_TRUE);
            if (ret != HI_SUCCESS)
            {
                err_print("Set ss_mpi_venc_enable_svc failed for %#x chn =%d\n", ret, venc_chn);
                LOG_ERROR("Set ss_mpi_venc_enable_svc failed for %#x chn =%d", ret, venc_chn);
            }
            ...
            ret = ss_mpi_venc_enable_svc(venc_chn, HI_FALSE);
            if (ret != HI_SUCCESS)
            {
                err_print("Set ss_mpi_venc_enable_svc failed for %#x chn =%d\n", ret, venc_chn);
                LOG_ERROR("Set ss_mpi_venc_enable_svc failed for %#x chn =%d", ret, venc_chn);
