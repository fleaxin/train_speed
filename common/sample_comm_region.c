...
td_s32 sample_region_create_overlay(td_s32 handle_num)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_create failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_create failed with %#x!", ret);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
td_s32 sample_region_destroy(ot_rgn_handle handle)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_destroy failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_destroy failed with %#x!", ret);
        return TD_FAILURE;
    }
...
td_s32 sample_region_attach_to_chn(ot_rgn_handle handle, ot_mpp_chn *chn, ot_rgn_chn_attr *chn_attr)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_attach_to_chn failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_attach_to_chn failed with %#x!", ret);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
td_s32 sample_region_detach_from_chn(ot_rgn_handle handle, ot_mpp_chn *chn)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_detach_from_chn failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_detach_from_chn failed with %#x!", ret);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
td_s32 sample_region_set_bit_map(ot_rgn_handle handle, ot_bmp *bitmap)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_set_bit_map failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_set_bit_map failed with %#x!", ret);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
td_s32 sample_region_get_up_canvas_info(ot_rgn_handle handle, ot_rgn_canvas_info *canvas_info)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_get_canvas_info failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_get_canvas_info failed with %#x!", ret);
        return TD_FAILURE;
    }

    ret = ss_mpi_rgn_update_canvas(handle);
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_rgn_update_canvas failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_rgn_update_canvas failed with %#x!", ret);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
td_s32 sample_comm_region_create(td_s32 handle_num, ot_rgn_type type)
{
    ...
    if (handle_num <= 0 || handle_num > 16) { /* 16:max_num */
        err_print("handle_num is illegal %d!\n", handle_num);
LOG_ERROR("handle_num is illegal %d!", handle_num);
        return TD_FAILURE;
    }
    if (type < 0 || type >= OT_RGN_BUTT) {
        err_print("type is illegal %d!\n", type);
LOG_ERROR("type is illegal %d!", type);
        return TD_FAILURE;
    }
    switch (type) {
        case OT_RGN_OVERLAY:
    ...
    if (ret != TD_SUCCESS) {
        err_print("sample_comm_region_create failed! handle_num%d,type:%d!\n", handle_num, type);
LOG_ERROR("sample_comm_region_create failed! handle_num%d,type:%d!", handle_num, type);
        return TD_FAILURE;
    }
    return ret;
}
...
LOG_ERROR("type is illegal %d!", type);
    ...
    for (i = min_handle; i < min_handle + handle_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("sample_comm_region_destroy failed!\n");
        }
    }
    return TD_SUCCESS;
}
    ...
    ot_rgn_chn_attr *chn_attr, ot_mpp_chn *mpp_chn)
{
    ...
    for (i = OVERLAY_MIN_HANDLE; i < OVERLAY_MIN_HANDLE + handle_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("sample_region_attach_to_chn failed!\n");
...
LOG_ERROR("type is illegal %d!", type);
    ...
    if (sample_comm_check_min(min_handle) != TD_SUCCESS) {
        err_print("min_handle(%d) should be in [0, %d).\n", min_handle, OT_RGN_HANDLE_MAX);
LOG_ERROR("min_handle(%d) should be in [0, %d).", min_handle, OT_RGN_HANDLE_MAX);
        return TD_FAILURE;
    }
    for (i = min_handle; i < min_handle + handle_num; i++) {
        ret = sample_region_detach_from_chn(i, mpp_chn);
        if (ret != TD_SUCCESS) {
            err_print("sample_region_detach_from_chn failed! handle:%d\n", i);
LOG_ERROR("sample_region_detach_from_chn failed! handle:%d", i);
        }
    }
    return TD_SUCCESS;
}
...
td_s32 sample_comm_region_set_bit_map(ot_rgn_handle handle, const td_char *bmp_path)
{
    ...
    if (handle < OVERLAY_MIN_HANDLE + 2) { /* 2: create clut2 region */
        ...
        if (ret != TD_SUCCESS) {
            err_print("region_mst_load_bmp failed!handle\n");
        }
    ...
    if (ret != TD_SUCCESS) {
        err_print("sample_region_set_bit_map failed!handle:%d\n", handle);
LOG_ERROR("sample_region_set_bit_map failed!handle:%d", handle);
    }
    free(load_bmp_info.bmp.data);
    return ret;
}
...
LOG_ERROR("ss_mpi_rgn_get_canvas_info failed with %#x!", ret);
    ...
    if (ret != TD_SUCCESS) {
        err_print("region_mst_update_canvas failed with %#x!\n", ret);
LOG_ERROR("region_mst_update_canvas failed with %#x!", ret);
        return TD_FAILURE;
    }
