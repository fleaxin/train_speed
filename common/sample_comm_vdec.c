    ...
    vdec_thread_param *thread_param, td_s32 used_bytes)
{
    ...
    if (*read_len > thread_param->min_buf_size) {
        err_print("chn %d read_len %d is bigger than buf_size %u!\n", thread_param->chn_id, *read_len,
    ...
    if (find_start == TD_FALSE) {
        err_print("chn %d can not find H264 start code! read_len %d, used_bytes %d!\n", thread_param->chn_id,
            *read_len, used_bytes);
    }
    ...
    vdec_thread_param *thread_param, td_s32 used_bytes)
{
    ...
    if (find_start == TD_FALSE) {
        err_print("chn %d can not find MPEG4 start code! read_len %d, used_bytes %d!\n", thread_param->chn_id,
            *read_len, used_bytes);
    }
    if (find_end == TD_FALSE) {
    ...
    vdec_thread_param *thread_param, td_s32 used_bytes)
{
    ...
    if (find_start == TD_FALSE) {
        err_print("chn %d can not find H265 start code! read_len %d, used_bytes %d!\n", thread_param->chn_id,
            *read_len, used_bytes);
    }
    if (find_end == TD_FALSE) {
    ...
    vdec_thread_param *thread_param, td_s32 used_bytes)
{
    ...
    *read_len = i + 2; /* 2:offset */
    ...
    if (find_start == TD_FALSE) {
        err_print("chn %d can not find JPEG start code! read_len %d, used_bytes %d!\n", thread_param->chn_id,
            *read_len, used_bytes);
    }
    return start;
}
    ...
    td_char *c_stream_file, td_u32 arr_len)
{
    if (thread_param->c_file_path == TD_NULL || thread_param->c_file_name == TD_NULL) {
        err_print("chn %d stream file path or stream file name is NULL\n", thread_param->chn_id);
LOG_ERROR("chn %d stream file path or stream file name is NULL", thread_param->chn_id);
        return TD_FAILURE;
    }

    if (arr_len <= 1) {
        err_print("chn %d arr length might be overflow\n", thread_param->chn_id);
LOG_ERROR("chn %d arr length might be overflow", thread_param->chn_id);
        return TD_FAILURE;
    }

    if (snprintf_s(c_stream_file, arr_len, arr_len - 1, "%s/%s", thread_param->c_file_path,
        thread_param->c_file_name) < 0) {
        err_print("chn %d config stream file failed!\n", thread_param->chn_id);
LOG_ERROR("chn %d config stream file failed!", thread_param->chn_id);
        return TD_FAILURE;
    }

    if (thread_param->min_buf_size <= 0) {
        err_print("chn %d min_buf_size should greater than zero!\n", thread_param->chn_id);
LOG_ERROR("chn %d min_buf_size should greater than zero!", thread_param->chn_id);
        return TD_FAILURE;
    }

    if (thread_param->fps <= 0 || thread_param->fps > 300) { /* 0~300:frame rate limit */
        err_print("chn %d fps should be [1, 300]!\n", thread_param->chn_id);
LOG_ERROR("chn %d fps should be [1, 300]!", thread_param->chn_id);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
...
td_void *sample_comm_vdec_send_stream(td_void *args)
{
    ...
    if (path == TD_NULL) {
        err_print("chn %d Invalid stream path. Please check!\n", thread_param->chn_id);
        LOG_ERROR("chn %d Invalid stream path. Please check!", thread_param->chn_id);
        return (td_void *)(TD_FAILURE);
    }

    fp_strm = fopen(path, "rb");
    if (fp_strm == TD_NULL) {
        err_print("chn %d can't open file %s in send stream thread!\n", thread_param->chn_id, c_stream_file);
        LOG_ERROR("chn %d can't open file %s in send stream thread!", thread_param->chn_id, c_stream_file);
        goto end1;
    }
    ...
    if (buf == TD_NULL) {
        err_print("chn %d can't alloc %d in send stream thread!\n",thread_param->chn_id, thread_param->min_buf_size);
        LOG_ERROR("chn %d can't alloc %d in send stream thread!",thread_param->chn_id, thread_param->min_buf_size);
        goto end;
    }
    fflush(stdout);
...
td_s32 sample_comm_config_ldy_attr(td_s32 i, td_u32 height)
{
    ...
    if (g_vdec_line_ldy_en == TD_TRUE) {
        ...
        check_chn_return(ss_mpi_vdec_set_display_mode(i, OT_VIDEO_DISPLAY_MODE_PLAYBACK), i,
...
td_s32 sample_comm_vdec_start(td_s32 chn_num, sample_vdec_attr *sample_vdec, td_u32 arr_len)
{
    ...
    check_null_ptr_return(sample_vdec);
    if (arr_len > OT_VDEC_MAX_CHN_NUM) {
        err_print("array size(%u) of chn_attr need < %u!\n", arr_len, OT_VDEC_MAX_CHN_NUM);
        LOG_ERROR("array size(%u) of chn_attr need < %u!", arr_len, OT_VDEC_MAX_CHN_NUM);
        return TD_FAILURE;
    }
