 ...
 * function :mpi_init
 */
hi_s32 sample_common_ive_mpi_init(hi_void)
{
    hi_s32 ret;
...
hi_s32 sample_common_ive_read_file(hi_svp_img *img, FILE *fp)
{
    ...
    sample_svp_check_exps_return(img == HI_NULL, ret, SAMPLE_SVP_ERR_LEVEL_DEBUG, "img can't be null\n");
    sample_svp_check_exps_return(fp == HI_NULL, ret, SAMPLE_SVP_ERR_LEVEL_DEBUG, "fp can't be null\n");
...
hi_u32 sample_common_ive_calc_stride(hi_u32 width, hi_u8 align)
{
    ...
    sample_svp_check_exps_return((width > HI_SAMPLE_IVE_MAX_WIDTH) || (width < 1), HI_ERR_IVE_ILLEGAL_PARAM,
        SAMPLE_SVP_ERR_LEVEL_DEBUG, "width(%u) must be in [1, %u]", width, HI_SAMPLE_IVE_MAX_WIDTH);
    ...
    hi_u16 *thresh)
{
    hi_u32 i;
    hi_u16 num;
    hi_u16 thr = blob->info.bits.cur_area_threshold;
    do {
        num = 0;
        thr += area_thr_step;
 ...
 */
static hi_void sample_common_ive_is_rect_valid(ot_sample_svp_rect_info *rect, hi_u32 num, hi_bool *valid)
{
    hi_u32 j, k;
 ...
 */
hi_s32 sample_common_ive_blob_to_rect(hi_ive_ccblob *blob, ot_sample_svp_rect_info *rect,
    hi_u16 rect_max_num, hi_u16 area_thr_step, hi_sample_src_dst_size src_dst_size)
{
    ...
    for (i = 0; i < blob->info.bits.rgn_num; i++) {
        ...
        sample_svp_check_exps_return(num > (OT_SVP_RECT_NUM - 1), HI_FAILURE,
    ...
    hi_bool is_mmz_cached)
{
    ...
    for (c = 0; (c < loop_info->loop_c) && (c < HI_MAX_LOOP_IMG_H) && (c < OT_SVP_IMG_STRIDE_NUM); c++) {
        size += img->stride[0] * loop_info->loop_h[c] * loop_info->ele_size;
        img->stride[c] = img->stride[0];
    }

    if (is_mmz_cached == HI_FALSE) {
        ret = ss_mpi_sys_mmz_alloc((hi_phys_addr_t *)&img->phys_addr[0], (hi_void **)&virt_addr,
            HI_NULL, HI_NULL, size);
    } else {
        ret = ss_mpi_sys_mmz_alloc_cached((hi_phys_addr_t *)&img->phys_addr[0], (hi_void **)&virt_addr,
            HI_NULL, HI_NULL, size);
    }
    ...
    if (img->type != OT_SVP_IMG_TYPE_U8C3_PACKAGE) {
        for (c = 1; (c < loop_info->loop_c) && (c < HI_MAX_LOOP_IMG_H) && (c < OT_SVP_IMG_STRIDE_NUM); c++) {
    ...
    hi_u32 width, hi_u32 height, hi_bool is_mmz_cached)
{
    ...
    sample_svp_check_exps_return(width > HI_SAMPLE_IVE_MAX_HEIGHT, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "width(%u) must be in [1, %u]!\n", width, HI_SAMPLE_IVE_MAX_HEIGHT);
...
hi_s32 sample_common_ive_create_mem_info(hi_svp_mem_info *mem_info, hi_u32 size)
{
    ...
    mem_info->size = size;
    ret = ss_mpi_sys_mmz_alloc((hi_phys_addr_t *)&mem_info->phys_addr, (hi_void **)&virt_addr, HI_NULL, HI_NULL, size);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
