...
//add by lewis
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_imx347_slave_12bit_1080p_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate  = MIPI_DATA_RATE_X1,
    .img_rect   = {0, 0, FHD_WIDTH, HEIGHT_1080},
    .mipi_attr  = {
        DATA_TYPE_RAW_12BIT,
        OT_MIPI_WDR_MODE_NONE,
        {4, 6, -1, -1, -1, -1, -1, -1}
...
//added by lewis
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev0_attr = {
    .devno = 0, /* dev3 */
    .input_mode = INPUT_MODE_MIPI,
    .data_rate  = MIPI_DATA_RATE_X1,
    .img_rect   = {0, 2, FHD_WIDTH, HEIGHT_1080},
    .mipi_attr  = {
        DATA_TYPE_RAW_12BIT,
        OT_MIPI_WDR_MODE_NONE,
        {0, 2, -1, -1, -1, -1, -1, -1}
...
//added by lewis
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev1_attr = {
    .devno = 1, /* dev3 */
    .input_mode = INPUT_MODE_MIPI,
    .data_rate  = MIPI_DATA_RATE_X1,
    .img_rect   = {0, 2, FHD_WIDTH, HEIGHT_1080},
...
//added by lewis
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev2_attr = {
    .devno = 2, /* dev2 */
    .input_mode = INPUT_MODE_MIPI,
    .data_rate  = MIPI_DATA_RATE_X1,
    .img_rect   = {0, 0, FHD_WIDTH, HEIGHT_1080},
...
//added by lewis
static combo_dev_attr_t g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev3_attr = {
    .devno = 3, /* dev3 */
    .input_mode = INPUT_MODE_MIPI,
    .data_rate  = MIPI_DATA_RATE_X1,
    .img_rect   = {0, 2, FHD_WIDTH, HEIGHT_1080},
...
static td_void sample_comm_vi_get_mipi_attr(sample_sns_type sns_type, combo_dev_attr_t *combo_attr)
{
    ...
    switch (sns_type) {
        ...
        case SONY_IMX347_SLAVE_MIPI_1080P_30FPS_12BIT:
            ob_height = IMX347_OB_HEIGHT_END;
            (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                &g_mipi_2lane_chn0_sensor_imx347_slave_12bit_1080p_nowdr_attr, sizeof(combo_dev_attr_t));//add by lewis
            break;
                                                      ...
                                                      combo_dev_attr_t *combo_attr)
{
    ...
    switch (sns_type) {
        ...
        case SONY_IMX347_SLAVE_MIPI_1080P_30FPS_12BIT:
            ...
            if (vi_dev == 2) { /* dev2 */
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev2_attr, sizeof(combo_dev_attr_t));
            }
            else if (vi_dev == 3) { /* dev3 */
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev3_attr, sizeof(combo_dev_attr_t));
            }
            else if (vi_dev == 0) { /* dev0 */
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev0_attr, sizeof(combo_dev_attr_t));
            }
            else if (vi_dev == 1) { /* dev1 */
                (td_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_2lane_chn0_sensor_imx347_slave_12bit_4m_nowdr_dev1_attr, sizeof(combo_dev_attr_t));
            }
            break;
...
td_s32 sample_comm_vi_set_vi_vpss_mode(ot_vi_vpss_mode_type mode_type, ot_vi_video_mode video_mode)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("set vi vpss mode failed!\n");
        return TD_FAILURE;
    }

    ret = ss_mpi_sys_set_vi_video_mode(video_mode);
    if (ret != TD_SUCCESS) {
        err_print("set vi video mode failed!\n");
        return TD_FAILURE;
    }
...
LOG_ERROR("open %s failed!", MIPI_DEV_NAME);
        return TD_FAILURE;
    }
...
static td_s32 sample_comm_vi_start_mipi_rx(const sample_sns_info *sns_info, const sample_mipi_info *mipi_info)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("mipi rx set hs_mode failed!\n");
        return TD_FAILURE;
    }

    ret = sample_comm_vi_mipi_ctrl_cmd(mipi_info->mipi_dev, OT_MIPI_ENABLE_MIPI_CLOCK);
    if (ret != TD_SUCCESS) {
        err_print("devno %d enable mipi rx clock failed!\n", mipi_info->mipi_dev);
LOG_ERROR("devno %d enable mipi rx clock failed!", mipi_info->mipi_dev);
        return TD_FAILURE;
    }

    ret = sample_comm_vi_mipi_ctrl_cmd(mipi_info->mipi_dev, OT_MIPI_RESET_MIPI);
    if (ret != TD_SUCCESS) {
        err_print("devno %d reset mipi rx failed!\n", mipi_info->mipi_dev);
LOG_ERROR("devno %d reset mipi rx failed!", mipi_info->mipi_dev);
        return TD_FAILURE;
    }

    ret = sample_comm_vi_mipi_ctrl_cmd(sns_info->sns_clk_src, OT_MIPI_ENABLE_SENSOR_CLOCK);
    if (ret != TD_SUCCESS) {
        err_print("devno %d enable sensor clock failed!\n", sns_info->sns_clk_src);
LOG_ERROR("devno %d enable sensor clock failed!", sns_info->sns_clk_src);
        return TD_FAILURE;
    }

    ret = sample_comm_vi_mipi_ctrl_cmd(sns_info->sns_rst_src, OT_MIPI_RESET_SENSOR);
    if (ret != TD_SUCCESS) {
        err_print("devno %d reset sensor failed!\n", sns_info->sns_rst_src);
LOG_ERROR("devno %d reset sensor failed!", sns_info->sns_rst_src);
        return TD_FAILURE;
    }

    ret = sample_comm_vi_set_mipi_combo_attr(&mipi_info->combo_dev_attr);
    if (ret != TD_SUCCESS) {
        err_print("mipi rx set combo attr failed!\n");
        return TD_FAILURE;
    }

    ret = sample_comm_vi_set_mipi_ext_data_type_attr(&mipi_info->ext_data_type_attr);
    if (ret != TD_SUCCESS) {
        err_print("mipi rx set ext data attr failed!\n");
        return TD_FAILURE;
    }

    ret = sample_comm_vi_mipi_ctrl_cmd(mipi_info->mipi_dev, OT_MIPI_UNRESET_MIPI);
    if (ret != TD_SUCCESS) {
        err_print("devno %d unreset mipi rx failed!\n", mipi_info->mipi_dev);
LOG_ERROR("devno %d unreset mipi rx failed!", mipi_info->mipi_dev);
        return TD_FAILURE;
    }

    ret = sample_comm_vi_mipi_ctrl_cmd(sns_info->sns_rst_src, OT_MIPI_UNRESET_SENSOR);
    if (ret != TD_SUCCESS) {
        err_print("devno %d unreset sensor failed!\n", sns_info->sns_rst_src);
LOG_ERROR("devno %d unreset sensor failed!", sns_info->sns_rst_src);
        return TD_FAILURE;
    }
...
LOG_ERROR("devno %d reset mipi rx failed!", mipi_info->mipi_dev);
    ...
    if (ret != TD_SUCCESS) {
        err_print("devno %d disable mipi rx clock failed!\n", mipi_info->mipi_dev);
LOG_ERROR("devno %d disable mipi rx clock failed!", mipi_info->mipi_dev);
    }
...
LOG_ERROR("devno %d reset sensor failed!", sns_info->sns_rst_src);
    ...
    if (ret != TD_SUCCESS) {
        err_print("devno %d disable sensor clock failed!\n", sns_info->sns_clk_src);
LOG_ERROR("devno %d disable sensor clock failed!", sns_info->sns_clk_src);
    }
}
...
static td_s32 sample_comm_vi_start_dev(ot_vi_dev vi_dev, const ot_vi_dev_attr *dev_attr, const ot_vi_bas_attr *bas_attr)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi set dev attr failed with 0x%x!\n", ret);
LOG_ERROR("vi set dev attr failed with 0x%x!", ret);
        return TD_FAILURE;
    }
    ...
    if ((bas_attr->enable == TD_TRUE) && (vi_dev == 0)) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi set bas attr failed with 0x%x!\n", ret);
LOG_ERROR("vi set bas attr failed with 0x%x!", ret);
            return TD_FAILURE;
        }
    }
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi enable dev failed with 0x%x!\n", ret);
LOG_ERROR("vi enable dev failed with 0x%x!", ret);
        return TD_FAILURE;
    }
...
static td_void sample_comm_vi_stop_dev(ot_vi_dev vi_dev)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi disable dev failed with 0x%x!\n", ret);
LOG_ERROR("vi disable dev failed with 0x%x!", ret);
    }
}
...
static td_s32 sample_comm_vi_dev_bind_pipe(ot_vi_dev vi_dev, const ot_vi_bind_pipe *bind_pipe)
{
    ...
    for (i = 0; i < bind_pipe->pipe_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi dev(%d) bind pipe(%d) failed!\n", vi_dev, bind_pipe->pipe_id[i]);
LOG_ERROR("vi dev(%d) bind pipe(%d) failed!", vi_dev, bind_pipe->pipe_id[i]);
            goto exit;
        }
    }
...
exit:
    for (j = i - 1; j >= 0; j--) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi dev(%d) unbind pipe(%d) failed!\n", vi_dev, bind_pipe->pipe_id[j]);
LOG_ERROR("vi dev(%d) unbind pipe(%d) failed!", vi_dev, bind_pipe->pipe_id[j]);
        }
    }
    return TD_FAILURE;
}
...
static td_void sample_comm_vi_dev_unbind_pipe(ot_vi_dev vi_dev, const ot_vi_bind_pipe *bind_pipe)
{
    ...
    for (i = 0; i < bind_pipe->pipe_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi dev(%d) unbind pipe(%d) failed!\n", vi_dev, bind_pipe->pipe_id[i]);
LOG_ERROR("vi dev(%d) unbind pipe(%d) failed!", vi_dev, bind_pipe->pipe_id[i]);
        }
    }
}
...
static td_s32 sample_comm_vi_set_grp_info(const sample_vi_grp_info *grp_info)
{
    ...
    for (i = 0; i < grp_info->grp_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi set wdr fusion grp attr failed!\n");
            return TD_FAILURE;
        }
    }
...
static td_s32 sample_comm_vi_start_chn(ot_vi_pipe vi_pipe, const sample_vi_chn_info chn_info[], td_u32 chn_num)
{
    ...
    for (i = 0; i < chn_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi set chn(%d) attr failed with 0x%x!\n", vi_chn, ret);
LOG_ERROR("vi set chn(%d) attr failed with 0x%x!", vi_chn, ret);
            return TD_FAILURE;
        }

        ret = ss_mpi_vi_enable_chn(vi_pipe, vi_chn);
        if (ret != TD_SUCCESS) {
            err_print("vi enable chn(%d) failed with 0x%x!\n", vi_chn, ret);
LOG_ERROR("vi enable chn(%d) failed with 0x%x!", vi_chn, ret);
            return TD_FAILURE;
        }
    }
...
static td_s32 sample_comm_vi_stop_chn(ot_vi_pipe vi_pipe, const sample_vi_chn_info chn_info[], td_u32 chn_num)
{
    ...
    for (i = 0; i < chn_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi disable chn(%d) failed with 0x%x!\n", vi_chn, ret);
LOG_ERROR("vi disable chn(%d) failed with 0x%x!", vi_chn, ret);
            return TD_FAILURE;
        }
    }
...
static td_s32 sample_comm_vi_start_one_pipe(ot_vi_pipe vi_pipe, const sample_vi_pipe_info *pipe_info)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi create pipe(%d) failed with 0x%x!\n", vi_pipe, ret);
LOG_ERROR("vi create pipe(%d) failed with 0x%x!", vi_pipe, ret);
    ...
    if (pipe_info->pipe_need_start == TD_TRUE) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi start pipe(%d) failed with 0x%x!\n", vi_pipe, ret);
LOG_ERROR("vi start pipe(%d) failed with 0x%x!", vi_pipe, ret);
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi pipe(%d) start chn failed!\n", vi_pipe);
LOG_ERROR("vi pipe(%d) start chn failed!", vi_pipe);
...
static td_void sample_comm_vi_stop_one_pipe(ot_vi_pipe vi_pipe, const sample_vi_pipe_info *pipe_info)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi pipe(%d) stop chn failed!\n", vi_pipe);
LOG_ERROR("vi pipe(%d) stop chn failed!", vi_pipe);
    }

    ret = ss_mpi_vi_stop_pipe(vi_pipe);
    if (ret != TD_SUCCESS) {
        err_print("vi stop pipe(%d) failed with 0x%x!\n", vi_pipe, ret);
LOG_ERROR("vi stop pipe(%d) failed with 0x%x!", vi_pipe, ret);
    }

    ret = ss_mpi_vi_destroy_pipe(vi_pipe);
    if (ret != TD_SUCCESS) {
        err_print("vi destroy pipe(%d) failed with 0x%x!\n", vi_pipe, ret);
LOG_ERROR("vi destroy pipe(%d) failed with 0x%x!", vi_pipe, ret);
    }
}
...
td_s32 sample_comm_vi_start_vi(const sample_vi_cfg *vi_cfg)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("start mipi rx failed!\n");
...
td_s32 sample_comm_vi_mode_switch_start_vi(const sample_vi_cfg *vi_cfg, td_bool chg_resolution, const ot_size *size)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("sample_comm_vi_start_isp failed!\n");
        goto start_isp_failed;
    }
...
td_s32 sample_comm_vi_switch_isp_mode(const sample_vi_cfg *vi_cfg)
{
    ...
    for (i = 0; i < dev_num; i++) {
        for (j = 0; j < vi_cfg->bind_pipe.pipe_num; j++) {
            ...
            if (ret != TD_SUCCESS) {
                err_print("ss_mpi_isp_get_pub_attr failed!\n");
                sample_comm_vi_stop_isp(vi_cfg);
            }
            ret = ss_mpi_isp_set_pub_attr(vi_pipe, &pub_attr);
            if (ret != TD_SUCCESS) {
                err_print("ss_mpi_isp_set_pub_attr failed!\n");
                sample_comm_vi_stop_isp(vi_cfg);
            }
                                              ...
                                              sample_vi_user_frame_info *user_frame_info)
{
    ...
    if (vb_blk == OT_VB_INVALID_HANDLE) {
        err_print("ss_mpi_vb_get_blk err, size:%d\n", calc_cfg->vb_size);
    LOG_ERROR("ss_mpi_vb_get_blk err, size:%d", calc_cfg->vb_size);
        return TD_FAILURE;
    }
...
td_void sample_comm_vi_free_frame_blk(sample_vi_user_frame_info *user_frame_info)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_sys_munmap failure!\n");
    }

    ret = ss_mpi_vb_release_blk(vb_blk);
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vb_release_blk block 0x%x failure\n", vb_blk);
LOG_ERROR("ss_mpi_vb_release_blk block 0x%x failure", vb_blk);
    }
                                    ...
                                    sample_vi_user_frame_info user_frame_info[], td_s32 frame_cnt)
{
    ...
    if (pool_id == OT_VB_INVALID_POOL_ID) {
        err_print("ss_mpi_vb_create_pool failed!\n");
        return TD_FAILURE;
    }
                                                ...
                                                sample_vi_user_frame_info *user_frame_info, td_s32 blk_cnt)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("vi get pipe attr failed!\n");
        return ret;
    }
    ...
    if (ret != TD_SUCCESS) {
        err_print("get fpn frame vb failed!\n");
        return ret;
    }
    ...
    ot_isp_fpn_calibrate_attr *calibrate_attr, td_s32 calib_cnt)
{
    ...
    for (i = 0; i < calib_cnt; i++) {
        ...
        if (ret != TD_SUCCESS) {
            err_print("vi fpn calibrate failed!\n");
            return TD_FAILURE;
        }
    ...
    td_u32 luma_stride, td_u32 chroma_width, td_u32 chroma_height)
{
    ...
    if (temp == TD_NULL) {
        err_print("vi malloc failed!\n");
        return TD_FAILURE;
    }
    if (memset_s(temp, chroma_stride, 0, chroma_stride) != EOK) {
        err_print("vi memset_s failed!\n");
        free(temp);
        temp = TD_NULL;
        return TD_FAILURE;
    }
...
static td_s32 sample_comm_vi_get_user_pic_frame_info(ot_size *dst_size, sample_vi_user_frame_info *user_frame_info)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("get user pic frame vb failed!\n");
        return ret;
    }
...
static td_s32 sample_comm_vi_add_scale_task(ot_video_frame_info *src_frame, ot_video_frame_info *dst_frame)
{
    ...
    if (ret != TD_SUCCESS) {
        ...
        err_print("ss_mpi_vgs_end_job failed, ret:0x%x", ret);
        return TD_FAILURE;
    }
...
LOG_ERROR("open file \"%s\" failed!", frame_file);
        ret = TD_FAILURE;
        goto exit0;
    }
    ...
    if (ret != TD_SUCCESS) {
        err_print("add vgs scale task failed.\n");
    }
