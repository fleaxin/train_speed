...
{
    if (size == TD_NULL) {
        err_print("null ptr arg!\n");
        return TD_FAILURE;
    }

    if (pic_size >= PIC_BUTT) {
        err_print("illegal pic_size!\n");
        return TD_FAILURE;
    }
    ...
    for (i = 0; i < max_dev_num; ++i) {
        ...
        for (j = 0; j < max_chn_num; ++j) {
            ...
            if (ss_mpi_sys_set_mem_cfg(&mpp_chn, mmz_name) != TD_SUCCESS) {
                err_print("ss_mpi_sys_set_mem_cfg ERR!\n");
                return TD_FAILURE;
            }
        }
    }
...
td_s32 sample_comm_sys_init(const ot_vb_cfg *vb_cfg)
{
    ...
    if (vb_cfg == TD_NULL) {
        err_print("input parameter is null, it is invalid!\n");
        return TD_FAILURE;
    }

    ret = ss_mpi_vb_set_cfg(vb_cfg);
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vb_set_conf failed!\n");
        return TD_FAILURE;
    }

    ret = ss_mpi_vb_init();
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vb_init failed!\n");
        return TD_FAILURE;
    }

    ret = ss_mpi_sys_init();
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_sys_init failed!\n");
        ss_mpi_vb_exit();
        return TD_FAILURE;
    }
...
td_s32 sample_comm_sys_init_with_vb_supplement(const ot_vb_cfg *vb_conf, td_u32 supplement_config)
{
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vb_set_supplement_conf failed!\n");
        return TD_FAILURE;
    }
...
td_s32 sample_comm_vpss_bind_vi(ot_vpss_grp vpss_grp, ot_vpss_chn vpss_chn, ot_vi_pipe vi_pipe)
{
    ot_mpp_chn src_chn;
    ot_mpp_chn dest_chn;
...
td_s32 sample_comm_vpss_un_bind_vi(ot_vpss_grp vpss_grp, ot_vpss_chn vpss_chn, ot_vi_pipe vi_pipe)
{
    ot_mpp_chn src_chn;
    ot_mpp_chn dest_chn;
...
td_s32 sample_comm_vdec_bind_vi(ot_vdec_chn vdec_chn, ot_vi_pipe vi_pipe)
{
    ot_mpp_chn src_chn;
    ot_mpp_chn dest_chn;
    ...
    dest_chn.mod_id = OT_ID_VI;
    dest_chn.dev_id = vi_pipe;
    dest_chn.chn_id = 0;

    check_return(ss_mpi_sys_bind(&src_chn, &dest_chn), "ss_mpi_sys_bind(VDEC-VI)");

    return TD_SUCCESS;
}

td_s32 sample_comm_vdec_un_bind_vi(ot_vdec_chn vdec_chn, ot_vi_pipe vi_pipe)
{
    ot_mpp_chn src_chn;
    ot_mpp_chn dest_chn;
    ...
    check_return(ss_mpi_sys_unbind(&src_chn, &dest_chn), "ss_mpi_sys_bind(VDEC-VI)");
