...
extern "C"
{
    ...
    td_s32 sample_comm_vo_get_width_height(ot_vo_intf_sync intf_sync, td_u32 *width, td_u32 *height, td_u32 *frame_rate)
    {
        ...
        {
            err_print("vo intf_sync %d not support!\n", intf_sync);
            LOG_ERROR("vo intf_sync %d not support!", intf_sync);
            return TD_FAILURE;
        }
                                    ...
                                    const ot_vo_user_sync_info *sync_info, td_u32 dev_frame_rate)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("failed with %#x!\n", ret);
            LOG_ERROR("failed with %#x!", ret);
            return TD_FAILURE;
        }
        ...
        ss_mpi_vo_set_dev_frame_rate(vo_dev, dev_frame_rate);
    ...
    td_s32 sample_comm_vo_get_wnd_info(sample_vo_mode mode, sample_vo_wnd_info *wnd_info)
    {
        ...
        {
            err_print("vo mode %d not support!\n", mode);
            LOG_ERROR("vo mode %d not support!", mode);
            return TD_FAILURE;
        }
    ...
    td_s32 sample_comm_vo_bt1120_start(ot_vo_dev vo_dev, ot_vo_pub_attr *pub_attr)
    {
        ...
        {
            err_print("open file (%s) fail!\n", LT8618SX_DEV_NAME);
            LOG_ERROR("open file (%s) fail!", LT8618SX_DEV_NAME);
            return TD_FAILURE;
        }
        ...
        {
            err_print("ioctl (%s) set clk fail!\n", LT8618SX_DEV_NAME);
            LOG_ERROR("ioctl (%s) set clk fail!", LT8618SX_DEV_NAME);
        }

        if (ioctl(g_vo_lt8618sx_fd, LT_CMD_SETMODE, &lt_mode) < 0)
        {
            err_print("ioctl (%s) set mode fail!\n", LT8618SX_DEV_NAME);
            LOG_ERROR("ioctl (%s) set mode fail!", LT8618SX_DEV_NAME);
        }
...
td_s32 sample_comm_vo_bt1120_start(ot_vo_dev vo_dev, ot_vo_pub_attr *pub_attr)
{
    ...
    static const ot_vo_user_sync_info g_sample_comm_vo_sync_info[OT_VO_MAX_PHYS_DEV_NUM] = {
        {
            .user_sync_attr = {
                ...
                .vo_pll = {
                    /* if hdmi, set it by pixel clk and div mode */
                    .fb_div = 99, /* 99 fb div */
                    .frac = 0,
                    .ref_div = 2,   /* 2 ref div */
    ...
    static td_s32 sample_comm_vo_get_vo_layer_attr(const sample_vo_cfg *vo_config, ot_vo_video_layer_attr *layer_attr)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_get_width_height failed!\n");
            return ret;
        }
    ...
    td_s32 sample_comm_vo_start_vo(const sample_vo_cfg *vo_config)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_start_dev failed!\n");
            return ret;
        }
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_start get video layer failed!\n");
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_start set video layer failed!\n");
            (td_void) sample_comm_vo_stop_dev(vo_dev);
            return ret;
        }
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_start_chn failed!\n");
    ...
    td_s32 sample_comm_vo_start_pip(sample_vo_cfg *vo_config)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_start video layer failed!\n");
            (td_void) sample_comm_vo_stop_dev(vo_dev);
            return ret;
        }
    ...
    td_s32 sample_comm_vo_stop_vo(const sample_vo_cfg *vo_config)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_vo stop chn failed!\n");
            return ret;
        }
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_vo stop layer failed!\n");
            return ret;
        }
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_vo stop dev failed!\n");
            return ret;
        }
    ...
    td_s32 sample_comm_vo_stop_pip(const sample_vo_cfg *vo_config)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_pip stop chn failed!\n");
            return ret;
        }
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_pip stop layer failed!\n");
            return ret;
        }
    ...
    td_s32 sample_comm_vo_stop_layer_chn(sample_comm_vo_layer_cfg *vo_layer_config)
    {
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_layer_chn stop chn failed!\n");
            return ret;
        }
        ...
        if (ret != TD_SUCCESS)
        {
            err_print("sample_comm_vo_stop_layer_chn stop layer failed!\n");
            return ret;
        }
