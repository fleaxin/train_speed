                                           ...
                                           const ot_vpss_chn_attr *chn_attr, td_u32 chn_array_size)
{
    ...
    for (i = 0; i < (td_s32)chn_array_size; ++i) {
        if (chn_enable[i] == TD_TRUE) {
            ...
            if (ret != TD_SUCCESS) {
                err_print("ss_mpi_vpss_set_chn_attr failed with %#x\n", ret);
                LOG_ERROR("ss_mpi_vpss_set_chn_attr failed with %#x", ret);
                goto disable_chn;
            }
            
            ret = ss_mpi_vpss_enable_chn(grp, vpss_chn);
            if (ret != TD_SUCCESS) {
                err_print("ss_mpi_vpss_enable_chn failed with %#x\n", ret);
                LOG_ERROR("ss_mpi_vpss_enable_chn failed with %#x", ret);
                goto disable_chn;
            }
        }
    }
...
disable_chn:
    for (i = i - 1; i >= 0; i--) {
        if (chn_enable[i] == TD_TRUE) {
            ...
            if (ret != TD_SUCCESS) {
                err_print("ss_mpi_vpss_disable_chn VPSS[%d][%d]failed with %#x!\n",grp,vpss_chn, ret);
                LOG_ERROR("ss_mpi_vpss_disable_chn VPSS[%d][%d]failed with %#x!",grp,vpss_chn, ret);
            }
        }
    }
    return TD_FAILURE;
    ...
    const ot_vpss_grp_attr *grp_attr, const ot_vpss_chn_attr *chn_attr, td_u32 chn_array_size)
{
    ...
    if (chn_array_size < OT_VPSS_MAX_PHYS_CHN_NUM) {
        err_print("array size(%u) of chn_enable and chn_attr need > %u!\n",
            chn_array_size, OT_VPSS_MAX_PHYS_CHN_NUM);
        return TD_FAILURE;
    }
    ret = ss_mpi_vpss_create_grp(grp, grp_attr);
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vpss_create_grp(grp:%d) failed with %#x!\n", grp, ret);
        LOG_ERROR("ss_mpi_vpss_create_grp(grp:%d) failed with %#x!", grp, ret);
        return TD_FAILURE;
    }
    ret = ss_mpi_vpss_start_grp(grp);
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vpss_start_grp failed with %#x\n", ret);
        LOG_ERROR("ss_mpi_vpss_start_grp failed with %#x", ret);
        goto destroy_grp;
    }
...
stop_grp:
    ...
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vpss_stop_grp failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_vpss_stop_grp failed with %#x!", ret);
    }
destroy_grp:
    ret = ss_mpi_vpss_destroy_grp(grp);
    if (ret != TD_SUCCESS) {
        err_print("ss_mpi_vpss_destroy_grp failed with %#x!\n", ret);
LOG_ERROR("ss_mpi_vpss_destroy_grp failed with %#x!", ret);
    }
    return TD_FAILURE;
}
...
td_s32 sample_common_vpss_stop(ot_vpss_grp grp, const td_bool *chn_enable, td_u32 chn_array_size)
{
    ...
    if (chn_array_size < OT_VPSS_MAX_PHYS_CHN_NUM) {
        err_print("array size(%u) of chn_enable need > %u!\n", chn_array_size, OT_VPSS_MAX_PHYS_CHN_NUM);
        LOG_ERROR("array size(%u) of chn_enable need > %u!", chn_array_size, OT_VPSS_MAX_PHYS_CHN_NUM);
        return TD_FAILURE;
    }
